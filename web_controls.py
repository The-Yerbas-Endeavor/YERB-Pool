...
def _users_snapshot():
    ...
    for user in users:
        recent_diff = float(user.pop("accepted_diff", 0) or 0)
        user["hashrate"] = (
            (recent_diff / enhanced.admin.live.base.GHOSTRIDER_TARGET_FACTOR)
            * enhanced.admin.live.base.DIFF1_HASHES
            / window
        )
    return {"users": users, "hashrate_window_seconds": window}
...
def _inject_user_admin(html):
    if 'id="admin-users"' not in html:
        users_section = (
            ...
            '<div class="muted">Miner accounts, balances, live hashrate, worker activity, source IPs and administrative controls.</div></div>'
    ...
    if "/admin_users.js" not in html:
        extra = '''<style>
#admin-users{margin-top:14px}.user-toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:10px}.user-toolbar input{width:min(520px,80vw)}.admin-user-table-wrap{overflow-x:auto}.admin-user-table{min-width:1050px}.admin-user-table td{vertical-align:top}.user-small{font-size:11px;margin-top:4px}.admin-ip{margin-bottom:10px}.user-badge{display:inline-block;padding:3px 7px;border-radius:999px;border:1px solid #444;font-size:11px;font-weight:700}.user-ok{color:#9fe3a7;border-color:#376c41;background:#152719}.user-paused{color:#ffe08a;border-color:#6d5d2b;background:#29240f}.user-banned{color:#ffaaaa;border-color:#7b3737;background:#2b1515}.user-control-btn,.user-mini-btn{padding:6px 9px;font-size:11px}.user-mini-btn{margin-left:6px}.danger{background:#8a3434}button:disabled{opacity:.55;cursor:wait}
</style><script src="/admin_users.js?v=1"></script>'''
