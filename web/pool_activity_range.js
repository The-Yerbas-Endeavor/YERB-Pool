...
(function(){
  ...
  const PAGE_SIZE=25;
  const pages={ledger:1,payouts:1};
  ...
  function sectionFor(title){
    return [...document.querySelectorAll('main#app section')].find(section=>
      section.querySelector('h2')?.textContent.trim()===title
    )||null;
  }

  function pageButtons(kind,page,totalPages){
    const buttons=[];
    const start=Math.max(1,Math.min(page-2,totalPages-4));
    const end=Math.min(totalPages,Math.max(5,page+2));
    for(let p=start;p<=end;p++){
      buttons.push(`<button type="button" data-account-page="${p}" data-account-kind="${kind}" class="${p===page?'active':''}">${p}</button>`);
    }
    return buttons.join('');
  }

  function pager(kind,page,totalPages,totalRows){
    if(totalPages<=1){
      return `<div class="account-history-pager yerb-pagination-info" data-account-pager="${kind}" data-signature="${page}:${totalRows}" style="text-align:center;margin-top:14px">${totalRows.toLocaleString()} result${totalRows===1?'':'s'}</div>`;
    }
    return `<div class="account-history-pager yerb-pagination" data-account-pager="${kind}" data-signature="${page}:${totalRows}"><button type="button" data-account-page="${page-1}" data-account-kind="${kind}" ${page<=1?'disabled':''}>← Prev</button>${pageButtons(kind,page,totalPages)}<button type="button" data-account-page="${page+1}" data-account-kind="${kind}" ${page>=totalPages?'disabled':''}>Next →</button><span class="yerb-pagination-info">Page ${page} of ${totalPages} · ${totalRows.toLocaleString()} results</span></div>`;
  }

  function renderSection(kind,title){
    ...
    const totalPages=Math.max(1,Math.ceil(rows.length/PAGE_SIZE));
    pages[kind]=Math.max(1,Math.min(pages[kind]||1,totalPages));
    const page=pages[kind];
    const start=(page-1)*PAGE_SIZE;
    rows.forEach((row,index)=>{row.hidden=index<start||index>=start+PAGE_SIZE});

    const signature=`${page}:${rows.length}`;
    const existing=section.querySelector(`[data-account-pager="${kind}"]`);
    if(existing?.dataset.signature===signature) return true;
    existing?.remove();
    section.insertAdjacentHTML('beforeend',pager(kind,page,totalPages,rows.length));
    return true;
  }

  function refresh(){
    ensureStyles();
    renderSection('ledger','Ledger');
    renderSection('payouts','Payout History');
  }

  document.addEventListener('click',event=>{
    const button=event.target.closest('[data-account-page][data-account-kind]');
    if(!button||button.disabled) return;
    const kind=button.dataset.accountKind;
    const page=parseInt(button.dataset.accountPage||'1',10);
    if(!Object.prototype.hasOwnProperty.call(pages,kind)||!Number.isFinite(page)||page<1) return;
    pages[kind]=page;
    renderSection(kind,kind==='ledger'?'Ledger':'Payout History');
    sectionFor(kind==='ledger'?'Ledger':'Payout History')?.scrollIntoView({behavior:'smooth',block:'start'});
  });
