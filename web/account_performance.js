(function(){
  const workerMatch=location.pathname.match(/^\/worker\/([^/]+)$/);
  if(workerMatch){
    ...
    function renderWorkerLastShare(){
      ...
      if(!metrics) return;
      let card=document.getElementById('worker-last-share-metric');
      if(!card){
        card=document.createElement('div');
        card.className='hash-metric';
        card.id='worker-last-share-metric';
        card.innerHTML='<span>Last submitted share</span><strong>—</strong><small></small>';
        metrics.appendChild(card);
      }
      const value=card.querySelector('strong');
      const detail=card.querySelector('small');
      if(lastShareAt){
        value.textContent=ago(lastShareAt);
        detail.textContent=new Date(Number(lastShareAt)*1000).toLocaleString();
      } else {
        value.textContent='No shares yet';
        detail.textContent='No submitted share recorded';
      }
    }
    ...
    async function refreshWorkerLastShare(){
      ...
      refreshBusy=true;
      try{
        const info=await get('/api/worker/'+encodeURIComponent(workerId)+'/stats?hours=1&bucket=60').catch(()=>null);
        if(info) lastShareAt=Number(info.last_share_at||0);
    ...
    const observer=new MutationObserver(()=>renderWorkerLastShare());
    const app=document.getElementById('app');
    if(app) observer.observe(app,{childList:true,subtree:true});

    const style=document.createElement('style');
    style.textContent='#worker-hash-card .hash-metrics{grid-template-columns:repeat(5,minmax(130px,1fr))}@media(max-width:1100px){#worker-hash-card .hash-metrics{grid-template-columns:repeat(3,minmax(130px,1fr))}}@media(max-width:760px){#worker-hash-card .hash-metrics{grid-template-columns:repeat(2,minmax(130px,1fr))}}@media(max-width:520px){#worker-hash-card .hash-metrics{grid-template-columns:1fr}}';
    document.head.appendChild(style);

    setTimeout(refreshWorkerLastShare,0);
    setInterval(refreshWorkerLastShare,10000);
    return;
  }
