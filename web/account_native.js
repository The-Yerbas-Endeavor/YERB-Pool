(function(){
  ...
  function performanceMarkup(history,current,rangeKey,workerCount,workerSeries){
    ...
    const rejectionHref=`/shares?status=rejected&address=${encodeURIComponent(accountAddress)}`;
    return `<div class="hash-head"><div><h3>Miner Performance</h3><div class="muted small">Combined hashrate and individual worker performance on this payout address.</div></div><div class="hash-range">${Object.keys(HASH_RANGES).map(k=>`<button type="button" data-account-range="${k}" class="${k===rangeKey?'active':''}">${k}</button>`).join('')}</div></div>
      <div class="hash-metrics account-hash-metrics">
        ...
        <a class="hash-metric account-reject-link" href="${rejectionHref}" title="View rejected share details for this miner"><span>Accepted / Rejected</span><strong>${accepted.toLocaleString()} / ${rejected.toLocaleString()}</strong><small>${efficiency.toFixed(2)}% accepted · ${rejectRate.toFixed(2)}% rejected · view rejection details →</small></a>
  ...
  account=async function(){
    ...
    app.innerHTML=`<a class="back" href="/miners">← Miners</a><section><h2>Miner Account</h2><div>${addressLink(x.address)} &nbsp; ${explorerAddress(x.address)}</div><div class="account-overview-grid"><div class="hash-metric"><span>Mature Balance</span><strong>${coin(x.balance_atomic)}</strong><small>YERB available for payout</small></div><div class="hash-metric"><span>Immature Balance</span><strong>${coin(x.immature_balance_atomic)}</strong><small>YERB awaiting confirmations</small></div><div class="hash-metric"><span>Total Earned</span><strong>${coin(x.total_earned_atomic)}</strong><small>YERB credited to this miner</small></div><div class="hash-metric"><span>Total Paid</span><strong>${coin(x.total_paid_atomic)}</strong><small>YERB paid to this address</small></div></div></section><section><div class="chart-grid" style="grid-template-columns:1fr"><div class="chart-card" id="account-hash-card">${initial.history.length?performanceMarkup(initial.history,initial.current,rangeKey,workers.length,initial.workerSeries):'<div class="empty">No worker history recorded yet.</div>'}</div></div></section><section><h2>Workers</h2>${x.workers.length?table(['Worker','Accepted','Rejected','Last Seen'],x.workers.map(w=>`<tr><td><a href="/worker/${w.id}"><code>${esc(w.name)}</code></a><div class="small muted">Worker #${esc(w.id)}</div></td><td>${w.accepted_shares}</td><td>${w.rejected_shares}</td><td>${ago(w.last_seen_at)}</td></tr>`)):'<div class="empty">No workers.</div>'}</section><section><h2>Ledger</h2>${x.ledger.length?table(['Time','Type','Amount','Block','Note'],x.ledger.map(l=>`<tr><td>${when(l.ts)}</td><td>${status(l.entry_type)}</td><td>${coin(l.amount_atomic)} YERB</td><td>${l.block_id??'—'}</td><td>${esc(l.note||'')}</td></tr>`)):'<div class="empty">No ledger entries.</div>'}</section><section><h2>Payout History</h2>${x.payouts.length?table(['ID','Status','Amount','Sent','TXID'],x.payouts.map(p=>`<tr><td>#${p.id}</td><td>${status(p.status)}</td><td>${coin(p.amount_atomic)} YERB</td><td>${when(p.sent_at)}</td><td>${explorerTx(p.txid)}</td></tr>`)):'<div class="empty">No payouts.</div>'}</section>`;

    const accountIdentity=app.querySelector('section > div');
    if(accountIdentity) accountIdentity.insertAdjacentHTML('beforeend',` &nbsp; <a href="/api/export/account/${encodeURIComponent(x.address)}.csv" download>Download CSV ↓</a>`);

    const style=document.createElement('style');
    style.textContent=`.account-overview-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:14px}.account-overview-grid .hash-metric{min-width:0}#account-hash-card .account-hash-metrics{grid-template-columns:repeat(5,minmax(130px,1fr))}#account-hash-card{min-height:520px}.account-reject-link{display:block;color:inherit;text-decoration:none;cursor:pointer}.account-reject-link:hover{border-color:#65c466;background:#172019;text-decoration:none}.account-worker-chart .account-worker-line{fill:none;stroke-width:1.8;opacity:.92;vector-effect:non-scaling-stroke}.account-worker-chart .account-combined-line{stroke-width:4;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 0 6px rgba(75,168,255,.72)) drop-shadow(0 0 12px rgba(75,168,255,.38));vector-effect:non-scaling-stroke}.account-worker-chart .share-accepted{fill:${ACCEPTED_COLOR};opacity:.42}.account-worker-chart .share-rejected{fill:${REJECTED_COLOR};opacity:.82}.account-worker-chart .series-hidden{opacity:0}.account-worker-legend{align-items:center}.worker-series-toggle{display:inline-flex;align-items:center;gap:5px;border:0;background:transparent;color:inherit;padding:2px 5px;cursor:pointer;font:inherit;opacity:.45}.worker-series-toggle.active{opacity:1}.worker-series-swatch{display:inline-block;flex:0 0 auto;overflow:visible}.worker-series-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:6px}.account-worker-note{text-align:center;margin-top:5px}@media(max-width:1000px){.account-overview-grid{grid-template-columns:repeat(2,minmax(0,1fr))}#account-hash-card .account-hash-metrics{grid-template-columns:repeat(2,minmax(130px,1fr))}}@media(max-width:520px){.account-overview-grid,#account-hash-card .account-hash-metrics{grid-template-columns:1fr}}`;
