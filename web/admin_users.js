(function(){
  ...
  function render(users){
    ...
    root.querySelectorAll('[data-user-row]').forEach((row,index)=>{
      const user=users[index],cell=row.querySelector('.user-address');
      if(!user||!cell)return;
      const workerLinks=(user.workers||[]).map(w=>`<a href="/api/admin/export/worker/${encodeURIComponent(w.id)}.csv">${esc(w.name)} lifetime CSV</a>`).join(' · ');
      cell.insertAdjacentHTML('beforeend',`<div class="user-small" style="margin-top:7px"><a href="/api/admin/export/account/${encodeURIComponent(user.address)}.csv"><strong>Account lifetime CSV</strong></a>${workerLinks?`<div class="muted" style="margin-top:4px">${workerLinks}</div>`:''}</div>`);
    });
