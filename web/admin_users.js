(function(){
  ...
  function render(users){
    ...
    root.innerHTML=`<div class="user-toolbar"><input id="admin-user-search" type="search" placeholder="Search address, worker, IP or country"><span class="muted">${users.length.toLocaleString()} user${users.length===1?'':'s'}</span></div><div class="admin-user-table-wrap"><table class="admin-user-table"><thead><tr><th>Address</th><th>Current hashrate</th><th>24h avg hashrate</th><th>Current balance</th><th>Immature balance</th><th>Total paid</th><th>Worker count</th><th>IP addresses</th><th>Country</th><th>First seen</th><th>Last share</th><th>Last payout</th><th>Accepted</th><th>Rejected</th><th>Reject %</th><th>Payment status</th><th>Mining status</th></tr></thead><tbody>${users.map(u=>`<tr data-user-row data-search="${esc([u.address,u.country,(u.worker_names||[]).join(' '),(u.ips||[]).map(x=>x.ip_address).join(' ')].join(' ').toLowerCase())}"><td class="user-address"><a href="/account/${encodeURIComponent(u.address)}" target="_blank"><code>${esc(u.address)}</code></a>${u.worker_names?.length?`<div class="muted user-small">Workers: ${u.worker_names.map(esc).join(', ')}</div>`:''}</td><td><strong>${hashRate(u.hashrate)}</strong></td><td><strong>${hashRate(u.hashrate_24h)}</strong></td><td><strong>${atomic(u.balance_atomic)}</strong></td><td>${atomic(u.immature_balance_atomic)}</td><td>${atomic(u.total_paid_atomic)}</td><td><strong>${Number(u.worker_count||0).toLocaleString()}</strong><div class="muted user-small">${Number(u.active_workers||0).toLocaleString()} active</div></td><td class="user-ips">${ipList(u)}</td><td>${esc(u.country||'Unavailable')}${u.country_code?`<div class="muted user-small">${esc(u.country_code)}</div>`:''}</td><td>${when(u.created_at)}</td><td>${when(u.last_share_at)}<div class="muted user-small">${ago(u.last_share_at)}</div></td><td>${when(u.last_payout_at)}</td><td>${Number(u.accepted_shares||0).toLocaleString()}</td><td>${Number(u.rejected_shares||0).toLocaleString()}</td><td>${pct(u.rejection_percent)}</td><td>${u.enabled?'<span class="user-badge user-ok">Active</span>':'<span class="user-badge user-paused">Hold</span>'}<div style="margin-top:8px"><button class="user-control-btn ${u.enabled?'danger':''}" data-payment-address="${esc(u.address)}" data-enabled="${u.enabled?'0':'1'}">${u.enabled?'Hold payments':'Resume payments'}</button></div></td><td>${u.suspended?'<span class="user-badge user-suspended">Suspended</span>':'<span class="user-badge user-ok">Active</span>'}<div style="margin-top:8px"><button class="user-control-btn ${u.suspended?'':'danger'}" data-mining-address="${esc(u.address)}" data-suspended="${u.suspended?'0':'1'}">${u.suspended?'Resume mining':'Suspend mining'}</button></div></td></tr>`).join('')}</tbody></table></div><div id="admin-user-message"></div>`;
