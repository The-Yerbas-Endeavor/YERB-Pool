(function(){
  ...
  async function renderWorkerDetail(){
    ...
    app.innerHTML=`<a class="back" href="/workers">← Workers</a>
      <section><div class="worker-detail-head"><div><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><h2 style="margin-bottom:0">${esc(x.name)}</h2>${x.active?'<span class="ok">● Active</span>':'<span class="muted">○ Idle</span>'}</div><div class="worker-detail-id small muted">Worker #${esc(x.id)} · First seen ${when(x.created_at)} · Last share ${ago(x.last_share_at)}</div><div style="margin-top:7px">${addressLink(x.address)} &nbsp; ${explorerAddress(x.address)}</div></div><div class="small muted">Last seen ${ago(x.last_seen_at)}</div></div>
