(function(){
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .worker-overview-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
      .worker-overview-grid .card{min-width:0;min-height:112px}
      ...
      @media(max-width:850px){.worker-overview-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
  ...
  async function renderWorkerDetail(){
    ...
    app.innerHTML=`<a class="back" href="/workers">← Workers</a>
      ...
      <div class="grid worker-overview-grid" style="margin-top:18px"><a class="card" href="#worker-performance"><div class="muted">Current Hashrate</div><div class="value">${hashRate(x.hashrate)}</div><div class="small muted">${Number(x.hashrate_window_seconds||120)}-second estimate</div></a><a class="card" href="#worker-performance"><div class="muted" id="worker-average-label">24h Average</div><div class="value" id="worker-average-value">${hashRate(x.average_hashrate)}</div><div class="small muted">Includes idle periods</div></a><a class="card" href="#recent-shares"><div class="muted">Accepted / Rejected</div><div class="value">${accepted.toLocaleString()} / ${rejected.toLocaleString()}</div><div class="small muted">${efficiency.toFixed(2)}% lifetime efficiency</div></a><a class="card" href="#recent-shares"><div class="muted">Last Share Difficulty</div><div class="value">${x.last_share_difficulty==null?'—':Number(x.last_share_difficulty).toFixed(6)}</div><div class="small muted">Most recently submitted share</div></a><a class="card" href="#recent-shares"><div class="muted">Last Submitted Share</div><div class="value">${x.last_share_at?ago(x.last_share_at):'—'}</div><div class="small muted">${x.last_share_at?when(x.last_share_at):'No shares submitted'}</div></a><a class="card" href="#blocks-found"><div class="muted">Blocks Found</div><div class="value">${Number(x.blocks_found_total||0).toLocaleString()}</div><div class="small muted">Most recent ${Math.min(25,(x.blocks_found||[]).length)} shown below</div></a></div></section>
