(function(){
  ...
  function findCard(){
    ...
    if(!c&&other)c=other;
    if(!c)return null;
    const grid=c.closest('.chart-grid');
    if(grid){grid.style.gridTemplateColumns='1fr';grid.classList.add('combined-hash-grid');}
    if(other&&other!==c)other.remove();
    c.classList.add('combined-hash-card');
    return c;
  }
  ...
  function makeSvg(history,samples,networkNow){
    ...
    let grid='';for(let i=0;i<=4;i++){const y=T+ih*i/4;grid+=`<line class="gridline" x1="${L}" y1="${y}" x2="${W-R}" y2="${y}"/><text class="axis pool-axis" x="4" y="${y+4}">${esc(hashRate(pmax*(1-i/4)))}</text><text class="axis net-axis" text-anchor="end" x="${W-4}" y="${y+4}">${esc(hashRate(nmax*(1-i/4)))}</text>`;}
    let labels='';const count=selected==='7D'?7:6;for(let i=0;i<count;i++){const ts=min+(max-min)*i/Math.max(1,count-1),x=px(ts);labels+=`<text class="axis" text-anchor="${i===0?'start':i===count-1?'end':'middle'}" x="${x}" y="${H-10}">${esc(label(ts))}</text>`;}
    const pa=rows.length?`${L},${T+ih} ${pp} ${W-R},${T+ih}`:'',na=rows.length?`${L},${T+ih} ${np} ${W-R},${T+ih}`:'';
    return `<div class="combined-chart-wrap"><svg class="combined-hash-chart" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">${grid}<polygon class="network-area" points="${na}"/><polygon class="pool-area" points="${pa}"/><polyline class="network-line" points="${np}"/><polyline class="pool-line" points="${pp}"/>${labels}<line class="hash-crosshair" x1="0" x2="0" y1="${T}" y2="${T+ih}"/></svg><div class="combined-hash-tooltip" hidden></div></div>`;
  }
  ...
  async function render(showLoading=false){
    ...
    try{
      ...
      const networkNow=Number(data.network_hashrate||0),poolNow=Number(data.pool_hashrate||0),serverSamples=Array.isArray(data.network_history)?data.network_history:[],browserSamples=sampleNetwork(networkNow),samples=serverSamples.length?serverSamples:browserSamples,r=RANGES[selected],cut=Math.floor(Date.now()/1000)-r.hours*3600,rangeSamples=samples.filter(x=>Number(x.ts)>=cut);
  ...
  start();
  // Intentionally no automatic chart redraw. The chart remains visually stable
  // until the user changes range or reloads the page.
})();
